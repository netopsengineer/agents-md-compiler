...
self-contained global, repository, or nested `AGENTS.md`, then check, install,
roll back, and verify it for an explicit target and startup context.

In scope:

- strict manifest parsing, deterministic path resolution, source validation;
- explicit lock refresh and locked verification;
- portable lock-format-2 pinning and rendered-format-2 output that preserves
  accepted source bytes exactly;
- strict format-1 lock migration and managed format-1 target upgrades;
- read-only state comparison against a target;
- explicit, atomic, backed-up, concurrency-safe installation and rollback;
- inspection of Codex model-visible startup input for active global and explicit
  project working-directory contexts.
...
- repository stack detection, target discovery, or multi-target manifests;
...
| `paths.py`        | Path-base resolution, tilde expansion, deployment state roots     |
...
| `lockfile.py`     | Portable format-2 locks and strict format-1 migration parsing     |
| `rendering.py`    | Format-version-2 rendering and managed-format recognition         |
| `state.py`        | Read-only state computation and precedence                        |
| `locking.py`      | Shared per-protected-path advisory file locking                   |
...
The following documents are frozen contracts. Change them only through the
documented schema or format-version procedure:
...
- generated output contains no timestamp, host name, process ID, random value,
  temporary path, package version, working directory, or resolved source path;
...
- equal manifest and source bytes produce identical format-2 lock and rendered
  bytes across checkout roots.
...
- derive advisory locks only from the resolved protected path under one shared
  distribution lock directory; never include bundle identity in that key;
- write a same-directory temporary file, flush, `fsync`, atomically replace,
  and `fsync` the directory where supported;
- record an exact backup and receipt under bundle-and-target deployment state
  before reporting success;
- accept legacy bundle-only receipts only for explicit validated rollback and
  never write new evidence to a legacy root.

Target and runtime context:

- one manifest selects one default target; an explicit `--target` may override it;
- do not infer repository roots, enumerate targets, or generate multiple outputs;
- exact rendered formats 1 and 2 are managed; unknown formats are unmanaged;
- `CURRENT` means selected-target byte equality, not visibility from every Codex
  startup directory;
- verify active global targets from an isolated empty directory;
- verify project and nested targets from explicit `--cwd` or the target parent;
- a sibling non-empty `AGENTS.override.md` shadows any selected `AGENTS.md`.
...
- Golden rendered output and golden lock files encode their named format version.
  A diff in a golden file is a format change, never an incidental test update.
...
- Treat `.agents/modules/project-contract.md` as the canonical project contract.
- Treat root `AGENTS.md` as generated output. Never edit it directly after the
  dogfood cutover.
- Keep `.agents/agents-md.toml` and its format-2 lock tracked.
- After changing the canonical contract, run `lock`, `validate`, `check`, and the
  repository Markdown and Unicode gates. Install explicitly only when the target
  is intentionally being refreshed.
- Verify visibility from the repository root with `verify-codex --cwd .` after an
  installed target change.
